...
namespace Dragonfly.Validator
{
    ...
    internal class DefaultValidatorFactory : IValidatorFactory
    {
        ...
        public T Resolve<T>() where T : class, IValidator
        {
            ...
            {
                throw new Exception($"Can not resolve validator for {typeof(T).GetGenericArguments()[0].Name}.");
            }
            throw new Exception($"Can not resolve validator {typeof(T).Name}.");
        }
        ...
        public bool IsDefaultValidator(Type validatorType)
        {
            ...
            {
                throw new ArgumentException(@"Validator type must be a class", nameof(validatorType));
            }
